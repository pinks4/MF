...
public class AuPro extends PageObjectModule {
    ...
    public void emailAuPro() throws Throwable {
		login.loginProau(); 
		  ...
		  String MFP = driver.getTitle();
			...
			if (MFP8.contains("Fool Pro » Forum: General Investing")) {
			...
			List<WebElement> element7 = driver.findElements(By.xpath("//h2[text()='General Investing']"));
      		if (element7.size() > 0) {
        	Reporter.log("General Investing Page Header Verified");
      		} else {
        	Reporter.log("General Investing Verification Failed");
			Assert.assertFalse(true);
      		}
			
			super.clickButton(read.getObject("Tab_AuProTL"));
            String MFP9 = driver.getTitle();
			if (MFP9.contains("Fool Pro » Forum: Foolish Banter")) {
			...
			List<WebElement> element8 = driver.findElements(By.xpath("//h2[text()='Foolish Banter!']"));
      		if (element8.size() > 0) {
        	Reporter.log("Foolish Banter Page Header Verified");
      		} else {
        	Reporter.log("Foolish Banter Verification Failed");
			Assert.assertFalse(true);
      		}
